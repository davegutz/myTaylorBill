...
class Begini(ConfigParser):
    def __init__(self, script_name, default_dict_):
        ...
        if Path(self.config_file_path).is_file():
            ...
            # Migrate legacy Archive folder to ArchiveFood
            if self.has_option("paths", "archive_folder"):
                old_archive = self.get("paths", "archive_folder")
                if old_archive.rstrip("/\\").endswith("Archive"):
                    self.set("paths", "archive_folder", os.path.join(os.path.dirname(old_archive.rstrip("/\\")), "ArchiveFood"))
            if self.has_option("paths", "last_photo_folder"):
                old_lpf = self.get("paths", "last_photo_folder")
                if old_lpf.rstrip("/\\").endswith("Archive"):
                    self.set("paths", "last_photo_folder", os.path.join(os.path.dirname(old_lpf.rstrip("/\\")), "ArchiveFood"))
            if self.has_option("paths", "last_photo"):
                old_lp = self.get("paths", "last_photo")
                if "/Archive/" in old_lp or "\\Archive\\" in old_lp:
                    self.set("paths", "last_photo", old_lp.replace("/Archive/", "/ArchiveFood/").replace("\\Archive\\", "\\ArchiveFood\\"))
...
class FoodAnalyzerApp:
    ...
    def action_import_photo(self):
        ...
        saves a copy to the ArchiveFood folder, and loads it for analysis (without OCR).
        ...
        archive_dir = self.cf.get_item("paths", "archive_folder", os.path.join(self.base_folder, "ArchiveFood"))
        try:
            os.makedirs(archive_dir, exist_ok=True)
        except Exception as e:
            self.log_status(f"Warning: could not create archive dir {archive_dir}: {e}")

        # Find candidate image files directly in search_dir (ignoring ArchiveFood subfolder)
